...
- 確定した速度ラベルは `overlay_hold_seconds` の間、画面上に保持されます

## デバッグモード

現在の `debug mode` は表示専用です。検知パラメータそのものは変更しません。
...
`detection_enabled` が OFF の間は、ライブ表示や各種ガイドは見えますが、実際の物体検知・追跡・速度計算は行いません。  
初期値は OFF です。
